...
# A large same-day move can make a stock LOOK like it's calmly resting on
# the 21-EMA (see: UCTT case study) when it actually just gapped through
# it. If today's move exceeds this, the setup is rejected rather than
# passed downstream as a BUY.
GAP_THRESHOLD_PCT = 5.0

# --- CONFIDENCE SCORE WEIGHTS ---
# Deterministic and reproducible on purpose — computed once here, never
# left for a downstream LLM step to guess at.
CONFIDENCE_BASE = 50
...
# ==========================================
# CONFIDENCE SCORE (deterministic — same formula every run, no LLM guessing)
# ==========================================
def calculate_confidence_score(reward_risk_ratio, dist_21, ext_50):
    """
    Every symbol reaching this function has already passed all gates, so
    this is a relative ranking score (1-100) among already-qualified
    setups, not a probability of success.
    """
    score = CONFIDENCE_BASE
    score += min(CONFIDENCE_RR_CAP, round(reward_risk_ratio * 5))
    score += round((PULLBACK_HIGH - abs(dist_21)) * 5)  # closer to 0% EMA distance = higher
...
def run_macro_analysis():
    ...
    for sym in symbols:
        try:
            ...
            # --- GATE 4b: SAME-DAY GAP CHECK ---
            # If today's close moved sharply vs. yesterday's close, the
            # 21-EMA proximity and low-volume-pullback readings above may
            # just be an artifact of the gap itself, not a genuine
            # multi-day pullback. Reject outright rather than pass the
            # ambiguity downstream to the LLM thesis-writer.
            prev_close = df['Close'].iloc[-2]
            quote_pct_change_today = ((current_price - prev_close) / prev_close) * 100
            gap_flag = abs(quote_pct_change_today) > GAP_THRESHOLD_PCT

            if gap_flag:
                skipped[sym] = f"gap contamination (today's move {quote_pct_change_today:.2f}%)"
                continue
            ...
            # --- CONFIDENCE SCORE ---
            # Computed here (not by the downstream LLM) so it's deterministic
            # and reproducible on identical input. See calculate_confidence_score.
            ...
            triggered_symbols.append({
                "symbol": sym,
                # action is hardcoded "BUY" because every symbol reaching this
                # point has already passed all 7 gates above, including the
                # gap check — there's no remaining condition for a downstream
                # LLM step to evaluate. Kept as an explicit field so the n8n
                # prompt has something concrete to pass through rather than
                # re-deciding it.
                "action": "BUY",
    ...
    if triggered_symbols:
        # Sort by confidence_score (composite of R:R, EMA proximity, extension)
        # rather than raw reward_risk_ratio alone.
