...
# Reject any single trade print that implies a larger % move from the
# previous trade than this. Guards against bad prints / crossed-quote
# artifacts, which are common on thin, illiquid micro-caps and can
# otherwise spike percent_change/VWAP distance and fire a false trigger.
...
def register_symbol_in_memory(target):
    ...
    Acquires data_lock itself — callers must NOT already hold data_lock
    when calling this, or the (non-reentrant) lock will deadlock."""
    ...
    while True:
        ...
        try:
            ...
            for target in latest_targets:
                sym = target["symbol"].replace('-', '.')
                # Check membership under a short-lived lock, then release it
                # BEFORE calling register_symbol_in_memory, which acquires
                # data_lock itself. Holding the lock across that call would
            ...
            for trade in data['data']:
                ...
                with data_lock:
                    if sym in market_data:
                        ...
                        # Reject implausible single-tick moves (bad prints /
                        # crossed-quote artifacts) before they can pollute
                        # cumulative volume, VWAP, or the order-flow delta.
                        if prev_tick > 0:
