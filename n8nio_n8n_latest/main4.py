...
def get_morning_watchlist():
    ...
    try:
        ...
        # 2. THE SAFETY SLICE
        # To prevent the Easypanel server from crashing and Yahoo from banning your IP,
        # we process the first 1,500 stocks. 
        tickers = all_tickers[:1500] 
        
        # Clean up Finnhub's formatting (e.g., BRK.B becomes BRK-B) for yfinance compatibility
        ...
        # 4. CALCULATE METRICS AND APPLY LOGIC GATES
        for sym in tickers:
            try:
                df = data[sym]
        ...
        print(f"Screener Complete! Found {len(top_40)} high-momentum matches.")
        return top_40

    except FileNotFoundError:
        print("CRITICAL ERROR: USSYMBOLS.json file not found in the directory!")
        return [{"symbol": "AAPL", "prev_close": 150}, {"symbol": "NVDA", "prev_close": 800}] # Fallback
    except json.JSONDecodeError:
        print("CRITICAL ERROR: USSYMBOLS.json is not formatted correctly.")
