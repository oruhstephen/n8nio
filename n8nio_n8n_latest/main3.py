...
def get_morning_watchlist():
    ...
    print("Fetching universe of US tickers...")
    
    try:
        # Get S&P 500 tickers for a highly liquid US universe
        sp500 = pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies')[0]['Symbol'].tolist()
        tickers = [t.replace('.', '-') for t in sp500] 
        
        print(f"Downloading 3-month historical data for {len(tickers)} symbols...")
        ...
        for sym in tickers:
            try:
                ...
                # --- THE LOGIC GATE (Your Custom Yahoo Screener) ---
                if percent_change >= 3.0 and avg_vol_3m >= 3000000 and current_price > 0 and rsi_14 > 0:
                    qualified_symbols.append({
                        ...
                        "rsi": rsi_14,
                        "avg_vol": avg_vol_3m,
                        "prev_close": prev_close # Saving this for the intraday math!
                    })
            except Exception:
                continue
    ...
    if data.get('type') == 'trade':
        ...
        if current_time - last_n8n_trigger >= 60:
            ...
            for sym, metrics in market_data.items():
                ...
                # We need some volume before VWAP becomes mathematically valid
                if cum_vol > 5000: 
